...
extension ARSCNView {
    func setUp(viewController: ViewController, session: ARSession) {
...
extension ARSCNView {
    ...
    func hitTestWithFeatures(_ point: CGPoint, coneOpeningAngleInDegrees: Float,
        ...
        for i in 0...features.__count {
            ...
            let crossProduct = originToFeature.cross(vector: ray.direction)
            let featureDistanceFromResult = crossProduct.length()
            
            let hitTestResult = ray.origin + (ray.direction * ray.direction.dot(vector: originToFeature))
            ...
            let angleBetweenRayAndFeature = acos(ray.direction.dot(vector: originToFeatureNormalized))
    ...
    func hitTestFromOrigin(origin: SCNVector3, direction: SCNVector3) -> FeatureHitTestResult? {
        ...
        for i in 0...features.__count {
            ...
            let crossProduct = originVector.cross(vector: direction)
            let featureDistanceFromResult = crossProduct.length()
        ...
        let hitTestResult = origin + (direction * direction.dot(vector: originToFeature))
