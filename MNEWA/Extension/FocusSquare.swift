...
class FocusSquare: SCNNode {
	...
	private func focusSquareNode() -> SCNNode {
        let arrow = SCNShape(path:  UIBezierPath.arrow(from: CGPoint(x: 1, y: 0), to: CGPoint(x: 3, y: 0),
                                                       tailWidth: 0.3, headWidth: 0.8, headLength: 0.5), extrusionDepth: 0)
       /* let node = SCNNode(geometry: arrow)
        node.name = "arrowNode"
        node.geometry?.firstMaterial?.isDoubleSided = true
        node.geometry?.firstMaterial?.diffuse.contents = UIColor.green
       // self.position = SCNVector3Make(locationNode.x, locationNode.y,locationNode.z)
        return node*/
        /*arrowNode.position.y = locationNode.y - 1.73
        arrowNode.eulerAngles.x = 0
